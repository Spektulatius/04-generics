 ...
 * Created on 10/6/17.
 */
public class GenericSimpleListImpl<T> implements GenericSimpleList<T> {

	private ListElement head;
	private int size;

	public GenericSimpleListImpl() {
		head = null;
	}
	 ...
	 */
	public GenericSimpleList<T> filter(GenericSimpleFilter<T> filter){
		GenericSimpleList<T> result = new GenericSimpleListImpl<>();
		for(T t : this){
			if(filter.include(t)){
				result.add(t);
			}
